...
$(KERNEL_CONFIG): $(KERNEL_REAL_PATH)/arch/$(TARGET_KERNEL_ARCH)/configs/$(KERNEL_DEFCONFIG)
	...
	mkdir -p $(KERNEL_OUT)
	$(MAKE) ARCH=$(TARGET_KERNEL_ARCH) -C $(KERNEL_REAL_PATH)  O=../$(KERNEL_OUT) $(KERNEL_CFLAGS) $(KERNEL_DEFCONFIG)
define sprd_create_user_config
...
ifeq ($(ZTE_KERNEL_CONFIG_ENABLE),true)
	$(call zte_diff_defconfig)
endif
...
$(TARGET_PREBUILT_KERNEL) : $(KERNEL_OUT) $(USER_CONFIG)
	$(MAKE) -C $(KERNEL_REAL_PATH) O=../$(KERNEL_OUT) ARCH=$(TARGET_KERNEL_ARCH) CROSS_COMPILE=$(KERNEL_CROSS_COMPILE) $(KERNEL_CFLAGS) -j${JOBS}
	@-mkdir -p $(KERNEL_MODULES_OUT)
	@-find $(TARGET_OUT_INTERMEDIATES) -name *.ko | xargs -I{} cp {} -v $(KERNEL_MODULES_OUT)

$(KERNEL_USR_HEADERS) : $(KERNEL_OUT)
	$(MAKE) -C $(KERNEL_REAL_PATH) O=../$(KERNEL_OUT) ARCH=$(TARGET_KERNEL_ARCH) CROSS_COMPILE=$(KERNEL_CROSS_COMPILE) $(KERNEL_CFLAGS) headers_install

kernelheader:
	mkdir -p $(KERNEL_OUT)
	$(MAKE) ARCH=$(TARGET_KERNEL_ARCH) -C $(KERNEL_REAL_PATH) O=../$(KERNEL_OUT) $(KERNEL_CFLAGS) headers_install
