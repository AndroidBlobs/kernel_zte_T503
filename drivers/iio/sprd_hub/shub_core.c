...
static void request_send_firmware(struct shub_data *sensor,
	...
	for (i = 0; i < MAX_COMPATIBLE_SENSORS; i++) {
		...
		snprintf(cali_info->chip_name, sizeof(cali_info->chip_name), "%s", sensor_firms[i]);
		success = 1;
		break;
	}
	if (!success)
...
static int set_als_calib_cmd(struct shub_data *sensor, u8 cmd, u8 id)
{
	...
	err = shub_send_command(sensor, ANDROID_LIGHT,
				SHUB_SET_CALIBRATION_DATA_SUBTYPE,
...
static ssize_t do_als_sensor_calibrate(struct device *dev,
				  ...
				  const char *buf, size_t count)
{
	...
	if (sensor->cal_cmd != CALIB_DATA_WRITE || sensor->cal_id != ANDROID_LIGHT) {
	...
	if (err < 0)
		pr_err("set_als_calib_cmd Fail!\n");

	return err < 0 ? err : count;
}

static ssize_t get_als_sensor_calib_status(struct device *dev,
				  struct device_attribute *attr, char *buf)
{
	...
	err = kernel_read(pfile, 0, raw_cali_data, CALIBRATION_DATA_LENGTH);
	if (err < 0) {
		pr_err("Error: file read failed\n");
		kfree(raw_cali_data);
		filp_close(pfile, NULL);
...
static struct device_attribute attributes[] = {
	...
	__ATTR(light_sensor_calibrator, 0644, get_als_sensor_calib_status, do_als_sensor_calibrate),
...
/******add for display chip info begin******/
static ssize_t chip_info_read(struct file *file, char __user *buffer, size_t count, loff_t *offset)
{
	int cnt;
	char buff[512] = {'\0'};
	...
	pr_info("chip_info_read enter\n");
	cnt = snprintf(buff, sizeof(buff), "%s\n%s\n%s\n%s\n%s\n%s\n",
		acc_cali_info.chip_name,
		gryo_cali_info.chip_name,
		mag_cali_info.chip_name,
		light_cali_info.chip_name,
		prox_cali_info.chip_name,
		pressure_cali_info.chip_name);

	pr_info("chip_info_read buf = %s\n", buff);
...
static int shub_probe(struct platform_device *pdev)
{
	...
	struct iio_dev *indio_dev;
	int error = 0;
	static struct proc_dir_entry *chip_info_proc = NULL;
	...
	chip_info_proc = proc_create("driver/sensor_chip_info", 0644, NULL, &chip_info_fops);
	if (chip_info_proc == NULL) {
		pr_err("sensor_chip_info proc fail!\n");
	}
