...
static void bq2560x_print_regs(void)
{
	int i;
	u8 value[12];

	for (i = 0; i < ARRAY_SIZE(value); i++) {
		bq2560x_read_reg(i, &(value[i]));
...
#ifdef CONFIG_CHARGER_SYV697
#define BQ_REG0B_PN_BQ25601		2
...
extern int syv697_probe(struct i2c_client *client, const struct i2c_device_id *id);

bool bq2560x_check_if_bq25601(void)
{
	u8 data, pn;

	bq2560x_read_reg(BQ2560X_REG_0B, &data);
	pn = data & REG0B_PN_MASK;
	pr_info("%s reg=0x%x, value=0x%x\n", __func__, BQ2560X_REG_0B, data);
	if (pn == (BQ_REG0B_PN_BQ25601 << REG0B_PN_SHIFT)) {
		pr_debug("is bq25601 pn=0x%x\n", pn);
		return true;
	}
	pr_debug("not bq25601 pn=0x%x\n", pn);
	return false;
}

bool bq2560x_check_if_syv697(void)
{
	u8 data, pn, dev;

	bq2560x_read_reg(BQ2560X_REG_0A, &data);
	pn = data & SYV_REG0A_PN_MASK;
	dev = data & SYV_REG0A_DEV_MASK;
	pr_info("%s reg=0x%x, value=0x%x\n", __func__, BQ2560X_REG_0A, data);

	if (pn == (SYV_REG0A_PN_SYV697 << SYV_REG0A_PN_SHIFT)) {
		pr_debug("maybe syv697, pn=0x%x\n", pn);
		if (dev == SYV_REG0A_DEV_SYV697) {
			pr_debug("is syv697, dev=0x%x\n", dev);
			return true;
		}
		pr_debug("not syv697, dev=0x%x\n", dev);
	}
	pr_debug("not syv697, pn=0x%x\n", pn);
			 ...
			 const struct i2c_device_id *id)
{
	...
	dev_err(&client->dev, "bq25601 probe enter\n");
...
#ifdef CONFIG_CHARGER_SYV697
	if (bq2560x_check_if_syv697() && !bq2560x_check_if_bq25601()) {
		pr_info("charger chip check done! not bq25601, it is syv697!\n");
		syv697_probe(client, id);
		return 0;
	}
#endif
	pr_info("charger chip check done! it is bq25601!\n");
	...
	dev_err(&client->dev, "bq25601 probe success\n");
	return 0;
}
