...
#ifdef ZTE_CHGR_SET_STEP_IBAT
int is_step_current = 0;
#define CURRENT_STEP_MA 400
#define RECOVERY_CURRENT_TIMEOUT_SEC 25
#endif
...
#ifdef ZTE_CHGR_SET_STEP_IBAT
		if (sprdbat_data->pdata->adp_cdp_cur == ZTE_CHGR_IBAT_LIMIT_MA) {
			sprdbat_data->bat_info.chg_current_type -= CURRENT_STEP_MA;
			is_step_current = 1;
			pr_info("is_step_current = %d chg_current_type=%d\n",
				is_step_current, sprdbat_data->bat_info.chg_current_type);
		}
#endif
...
#ifdef ZTE_CHGR_SET_STEP_IBAT
			if (sprdbat_data->pdata->adp_dcp_cur == ZTE_CHGR_IBAT_LIMIT_MA) {
				sprdbat_data->bat_info.chg_current_type -= CURRENT_STEP_MA;
				is_step_current = 1;
				pr_info("is_step_current = %d, chg_current_type=%d\n",
					is_step_current, sprdbat_data->bat_info.chg_current_type);
...
#ifdef ZTE_CHGR_SET_STEP_IBAT
	is_step_current = 0;
	sprdbat_data->bat_info.chg_current_type = sprdbat_data->pdata->adp_sdp_cur;
#endif
	sprdbat_data->battery_work_cycle_s = sprdbat_data->pdata->chg_polling_time;
...
static void sprdbat_update_capacty(void)
{
	...
	if (sprdbat_data->pdata->only_vol_mode) {
		...
		SPRDBAT_DEBUG("fgu_capacity orig: %d\n", fgu_capacity);
		fgu_capacity = DIV_ROUND_CLOSEST(fgu_capacity*101, 98);
		SPRDBAT_DEBUG("fgu_capacity new: %d\n", fgu_capacity);
...
#ifdef ZTE_CHGR_SET_STEP_IBAT
static void sprdbat_recovery_current_to_normal(void)
{
	struct timespec64 cur_time;

	if (is_step_current) {
		pr_info("is_step_current = %d\n", is_step_current);
		cur_time = ktime_to_timespec64(ktime_get_boottime());
		if (cur_time.tv_sec - sprdbat_data->bat_info.chg_start_time > RECOVERY_CURRENT_TIMEOUT_SEC) {
			sprdbat_data->bat_info.chg_current_type += CURRENT_STEP_MA;
			if (sprdbat_data->bat_info.chg_stop_flags == SPRDBAT_CHG_END_NONE_BIT) {
				sprd_ext_ic_op->set_charge_cur(sprdbat_data->bat_info.chg_current_type);
			}
			is_step_current = 0;
			pr_info("is_step_current = %d chg_current_type=%d\n",
				is_step_current, sprdbat_data->bat_info.chg_current_type);
...
static int sprdbat_adapt_plug_check(void)
{
	uint32_t vbat_vol = 0;
	uint32_t vchg_vol = 0;
	unsigned int poll_time =
	...
	vbat_vol = sprdbat_read_vbat_vol();
	vchg_vol = sprdchg_read_vchg_vol();

	pr_info("%s bat_current = %d vbat_vol = %d vchg_vol = %d\n",
		__func__, sprdbat_data->bat_info.bat_current, vbat_vol, vchg_vol);
	if (vbat_vol > 4200 && sprdbat_data->battery_work_cycle_s != poll_time_fast) {
		SPRDBAT_DEBUG("polling fast %d!\n", poll_time_fast);
		sprdbat_data->battery_work_cycle_s = poll_time_fast;
		sprdchg_timer_op->timer_disable();
		sprdchg_timer_op->timer_enable(poll_time_fast, PERIOD_TIME);
	} else if (vbat_vol < 4100 && sprdbat_data->battery_work_cycle_s != poll_time) {
		SPRDBAT_DEBUG("polling norm %d!\n", poll_time);
		sprdbat_data->battery_work_cycle_s = poll_time;
		sprdchg_timer_op->timer_disable();
		sprdchg_timer_op->timer_enable(poll_time, PERIOD_TIME);
...
#ifdef ZTE_CHGR_SET_STEP_IBAT
	sprdbat_recovery_current_to_normal();
#endif
...
#ifdef ZTE_CHGR_IUSB_LIMIT_MA
	pdata->adp_cdp_cur_limit = ZTE_CHGR_IUSB_LIMIT_MA;
	pdata->adp_dcp_cur_limit = ZTE_CHGR_IUSB_LIMIT_MA;
	SPRDBAT_DEBUG("ZTE_CHGR_IUSB_LIMIT_MA %d defined\n", pdata->adp_dcp_cur_limit);
#endif
#ifdef ZTE_CHGR_IBAT_LIMIT_MA
	pdata->adp_cdp_cur = ZTE_CHGR_IBAT_LIMIT_MA;
	pdata->adp_dcp_cur = ZTE_CHGR_IBAT_LIMIT_MA;
	SPRDBAT_DEBUG("ZTE_CHGR_IBAT_LIMIT_MA %d defined\n", pdata->adp_dcp_cur);
#endif
