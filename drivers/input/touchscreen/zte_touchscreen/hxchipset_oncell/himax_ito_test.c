...
void himax_ito_mul_baseC_log_func(void)
{
	...
	int len = 0;
	char *result_buff = NULL;
	int result_buff_len = 0;
/* char log_type_name[10]; */
	char temp[1000] = {0};
	struct file *fn = NULL;

	result_buff_len = (mutual_num + self_num) * 15 * sizeof(char);
	result_buff = kzalloc(result_buff_len, GFP_KERNEL);
	if (result_buff == NULL) {
		E("%s:Can not allocate result_buff Buffer\n", __func__);
		return;
	}
	I("%s:Entering!\n", __func__);
	...
	strlcat(result_buff, temp, result_buff_len);
...
void himax_ito_self_baseC_log_func(void)
{
	...
	result_buff_len = (mutual_num + self_num) * 10 * sizeof(char);
	result_buff = kzalloc(result_buff_len, GFP_KERNEL);
	if (result_buff == NULL) {
		E("%s:Can not allocate result_buff Buffer\n", __func__);
		return;
