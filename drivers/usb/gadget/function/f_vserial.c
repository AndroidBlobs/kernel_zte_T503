...
fail:
	pr_err("Allocate requests failed.\n");
	return ret;
}
...
requeue_req:
	...
	if (ret < 0) {
		pr_info("vser_read: usb_ep_dequeue\n");
...
static int vser_open(struct inode *ip, struct file *fp)
{
	...
	if (dev->open_count > 2) {
		dev->open_count--;
		pr_warn("too many user open vser.\n");
		spin_unlock_irqrestore(&dev->lock, flags);
		return -EBUSY;
	}
		      ...
		      const struct usb_ctrlrequest *ctrl)
{
	...
	if ((ctrl->bRequestType & USB_TYPE_MASK) == USB_TYPE_CLASS) {
		...
		case START_TEST:
			...
			if (vser_alloc_all_request(dev) < 0)
				pr_info("Vser allocate memory failed!\n");
			dev->test_mode |= 0x80;
			dev->total_size = parameter;
			pr_info("total size = 0x%x\n", parameter);
	...
	if (value >= 0) {
		...
		if (rc < 0)
			pr_warn("%s setup response queue error\n", __func__);
	}
...
ssize_t vser_iq_write(char *buf, size_t count)
{
	...
	pr_info("%s: buf:0x%p, count:0x%llx, epname:%s\n",
	...
	while (count > 0) {
		...
		if (ret < 0) {
			r = ret;
			pr_warn("%s:line %d wait event failed.\n",
				__func__, __LINE__);
			break;
		}
...
static void vser_test_works(struct work_struct *work)
{
	...
	uint32_t start_times = jiffies;

	total = dev->total_size;
	req_recv = dev->rx_req;

	pr_info("%s: start (%d, 0x%x)\n", __func__, dev->test_mode, total);
	if (!req_recv) {
		pr_info("receive request is NULL!\n");
		return;
	}
