...
static void sprd_musb_work(struct work_struct *work)
{
	...
	if (glue->vbus_active) {
		...
		*/
		...
		dev_info(glue->dev, "after first while boot_charging%d\n", boot_charging);
		if ((glue->dr_mode == USB_DR_MODE_PERIPHERAL &&
			!musb_sprd_is_connect_host(glue)) || boot_charging) {
			dev_info(glue->dev, "enter if case1\n");
			spin_lock_irqsave(&glue->lock, flags);
			glue->charging_mode = true;
			dev_info(glue->dev, "charging_mode is true\n");
			spin_unlock_irqrestore(&glue->lock, flags);
		...
		dev_info(glue->dev, "after second while\n");
		if (cnt <= 0) {
			dev_err(glue->dev,
	...
	dev_info(glue->dev, "enable_usb_irq\n");
