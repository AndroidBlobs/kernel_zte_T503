...
u8 binwin_gc(struct mmc_card *card)
{
		struct mmc_command cmd;
		int err = 0;

		memset(&cmd, 0, sizeof(struct mmc_command));
		cmd.opcode = MMC_REV_CMD_FOR_MANU;
		cmd.arg = 0x534d4900;
		cmd.flags = MMC_RSP_SPI_R1B | MMC_RSP_R1B | MMC_CMD_AC;
		cmd.busy_timeout = BW_GC_CMD_TIMEOUT_MS;

		err = mmc_wait_for_cmd(card->host, &cmd, 0);
		if (err) {
			pr_err("World ERROR : %d, send command %d\n", err, cmd.opcode);
			return 0;
		}
		pr_info("World CMD %d, response = %x\n", cmd.opcode, cmd.resp[0]);
		...
		cmd.arg = 0x55;
		cmd.flags = MMC_RSP_SPI_R1B | MMC_RSP_R1B | MMC_CMD_AC;
		cmd.busy_timeout = DEFAULT_CMD6_TIMEOUT_MS;
		...
		if (card->host->caps & MMC_CAP_WAIT_WHILE_BUSY) {
			pr_err("World ERROR : %d, send command timeout %d\n", err, cmd.opcode);
			return 0;
		}
		pr_info("World CMD %d, response = %x\n", cmd.opcode, cmd.resp[0]);
...
int enable_to_gc = 0;
static int set_binwin_gc(const char *val, struct kernel_param *kp)
{
	int rc;
	int ret = 0;
	struct mmc_card *card = p_card;

	if (!card) {
		pr_err("World %s: card is null\n", __func__);
		return ret;
	}

	if (!card) {
		pr_err("World %s: card host is null\n", __func__);
		return ret;
	}
	rc = param_set_int(val, kp);
	if (rc) {
		pr_err("World %s: error setting gc value %d\n", __func__, rc);
		return rc;
	}
	if (card != NULL
		&& card->cid.manfid == 0x0000F4
		&& card->ext_csd.fwrev[1] == 0x00
		&& card->ext_csd.fwrev[2] == 0x00
		&& card->ext_csd.fwrev[3] == 0x00
		&& card->ext_csd.fwrev[4] == 0x00
		&& card->ext_csd.fwrev[5] == 0x00
		&& card->ext_csd.fwrev[6] == 0x00
		&& card->ext_csd.fwrev[7] == 0x00
		&& card->cid.prod_name[0] == 'B'
		&& card->cid.prod_name[1] == 'W'
		&& card->cid.prod_name[2] == 'Z'
		&& card->cid.prod_name[3] == 'T'
		&& card->cid.prod_name[4] == '6'
		&& card->cid.prod_name[5] == '8') {
	pr_info("World %s:  BW G.C ENTER\n", __func__);
	mmc_get_card(card);
	binwin_gc(card);
	mmc_put_card(card);
	return ret;
	}
	pr_info("World %s:  CAN NOT FOUND BWZT68\n", __func__);
	return ret;
}

module_param_call(enable_to_gc, set_binwin_gc, NULL,
			&enable_to_gc, 0644);
