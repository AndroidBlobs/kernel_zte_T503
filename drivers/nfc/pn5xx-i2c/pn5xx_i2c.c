...
static irqreturn_t pn5xx_dev_clkreq_irq_handler(int irq, void *dev_id)
{
	NFC_DBG_MSG("clkreq_irq enter !\n");
		...
		size_t count, loff_t *offset)
{
	...
	if (!gpio_get_value(pn5xx_dev->irq_gpio)) {
		...
		enable_irq_wake(pn5xx_dev->client->irq);
		}
		...
		/*pn5xx_disable_irq(pn5xx_dev);*/
		NFC_DBG_MSG("wait_event_interruptible end! ret:%d\n", ret);
		if (ret) {
			int clkreqirq_gpio_value;

			clkreqirq_gpio_value = gpio_get_value(pn5xx_dev->clkreq_gpio);
			NFC_DBG_MSG("pn5xx_dev_read  pn5xx_dev->clkreq_gpio %d\n", clkreqirq_gpio_value);
			if (clkreqirq_gpio_value == 1) {
			__pm_wakeup_event(nfc_wake_lock, 5 * MSEC_PER_SEC);
			}
			goto fail;
		}
	...
	case PN5XX_SET_PWR:
	{
		if (arg == 2) {
		    ...
		    */
			...
			/*zte-modify quguotao*/
			if (!pn5xx_dev->clk_req_enabled) {
			enable_irq_wake(clk_irq);
			pn5xx_dev->clk_req_enabled = true;
			}
			/*zte-modify quguotao*/
#ifdef CONFIG_NFC_ENABLE_BB_CLK2
			if (clk_run == false) {
			...
			/*zte-modify quguotao*/
			if (pn5xx_dev->clk_req_enabled) {
			disable_irq_wake(clk_irq);
			pn5xx_dev->clk_req_enabled = false;
			}
			/*zte-modify quguotao*/
#ifdef CONFIG_NFC_ENABLE_BB_CLK2
			if (clk_run == true) {
...
#endif
#ifdef CONFIG_NFC_ENABLE_BB_CLK2
		pn5xx_dev->clkreq_gpio = of_get_named_gpio(of_node, "nxp,pn544-clkreq", 0);
		clk_irq = gpio_to_irq(pn5xx_dev->clkreq_gpio);
		if (clk_irq < 0) {
			pr_info("%s clkreq_gpio to clk_irq failed", __func__);
			goto err_clkreq_gpio;
		}

		ret = request_irq(clk_irq, pn5xx_dev_clkreq_irq_handler,
				  IRQF_TRIGGER_RISING, client->name, pn5xx_dev);
		if (ret) {
			NFC_ERR_MSG("request_irq clkreq_gpio failed\n");
			goto err_request_irq_failed;
		} else {
			/* test to see if clk_irq is really wakeup capable */
			ret = irq_set_irq_wake(clk_irq, 1);
			if (ret) {
				NFC_ERR_MSG("clk_irq %d cannot set wakeup (%d)\n", clk_irq, ret);
				ret = 0;
			} else {
				irq_set_irq_wake(clk_irq, 0);
				pn5xx_dev->clk_req_enabled = false;
			}
			NFC_DBG_MSG("request_irq  clkreq_gpio ok\n");
		}
#endif
