...
#include <linux/delay.h>
struct mutex mutex_spi_tx;/*yangchafeng add for spi transfer*/
...
static void zte_kernel_dump_fb(unsigned char *src_buf, u32 length)
{
	...
	snprintf(dumpFilename, sizeof(dumpFilename), "%s/sfd_kernel_dump%04d.raw", DUMP_DIR, dump_times++);
	p_file = filp_open(dumpFilename, O_CREAT | O_RDWR, 0);
	...
	pos = 0;
	vfs_write(p_file, src_buf, length, &pos);

	filp_close(p_file, NULL);
	set_fs(old_fs);
}
#endif/*end ZTE_DUMP_FB*/

unsigned char brightness_cmd[] = {0x51, 0x36};
unsigned char cmd_2c[] = {0x2c};
unsigned char brightness_data[] = {0x00, 0x55, 0xaa, 0xff, 0x08};
int mdss_spi_read_data(u8 reg_addr, u8 *data, u8 len)
{
	...
	mutex_lock(&mutex_spi_tx);
	gpio_set_value(panel->spi_cd_gpio, 0);
	spi->spi_dev->bits_per_word = 8;
	spi->spi_dev->max_speed_hz = 600000;
	spi_setup(spi->spi_dev);

	spi_message_init(&m);
	spi_message_add_tail(&t[0], &m);
	spi_message_add_tail(&t[1], &m);
	rc = spi_sync(spi->spi_dev, &m);

	spi->spi_dev->bits_per_word = 32;
	spi->spi_dev->max_speed_hz = 48000000;
	spi_setup(spi->spi_dev);
	gpio_set_value(panel->spi_cd_gpio, 1);
	mutex_unlock(&mutex_spi_tx);
...
#endif/*end CONFIG_ZTE_LCD_SPI_PANEL*/
...
#ifndef CONFIG_ZTE_LCD_SPI_PANEL
...
static void sprd_spi_flip_layer(struct sprd_adf_hwlayer *hwlayer)
{
	...
	if (layer_bpp == 32) {
		...
		#ifdef CONFIG_ZTE_LCD_SPI_PANEL
		zte_kernel_dump_fb(pframe, 153600);
		#endif
		if (pframe)
			exchange_pixels(pframe);
		else
...
static int sprd_spi_probe(struct platform_device *pdev)
{
	...
	mutex_init(&mutex_spi_tx);/*yangchafeng add for spi transfer*/

	/*
	if ( 0 ){
		mdss_spi_tx_command(&brightness_cmd[0]);
		mdss_spi_tx_parameter(&brightness_data[3], 1);
		mdelay(1000);
		mdss_spi_tx_command(&brightness_cmd[0]);
		mdss_spi_tx_parameter(&brightness_data[2], 1);
		mdelay(1000);
		mdss_spi_tx_command(&brightness_cmd[0]);
		mdss_spi_tx_parameter(&brightness_data[1], 1);
		mdelay(1000);
		mdss_spi_tx_command(&brightness_cmd[0]);
		mdss_spi_tx_parameter(&brightness_data[0], 1);
		mdelay(1000);
	...
	mdss_spi_tx_pixel(gImage_shuipingsao, 153600);
	mdss_spi_tx_command(&cmd_2c[0]);

	mdelay(2000);
	mdss_spi_tx_pixel(gImage_shuipingsaoh, 153600);
	mdss_spi_tx_command(&cmd_2c[0]);
	mdelay(2000);
