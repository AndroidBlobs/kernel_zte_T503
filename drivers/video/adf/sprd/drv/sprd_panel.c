...
static void esd_work_func(struct work_struct *work)
{
	...
	if (ret
		...
		if (g_zte_ctrl_pdata->zte_lcd_ctrl->is_lcd_esd_twice) {
			mdelay(200);
			pd->esd_work_start = false;
			pr_err("====== esd recovery start ========\n");
			disp_notifier_call_chain(DISP_EVENT_ESD_RECOVER, (void *)0);
...
static struct panel_info *of_parse_panel(struct device_node *panel_node)
{
	...
	#ifdef CONFIG_ZTE_LCD_CUSTOM_AP_PWM_BACKLIGHT
	if (zte_hw_boardid >= 0x01) {
		panel->is_oled = false;

		rc = of_parse_cmds(panel_node, &pcmds, "init-data-v2");
		if (!rc)
			panel->cmd_codes[CMD_CODE_INIT] = pcmds;
		else
			pr_err("init-data-v2 cmd not found\n");
...
int zte_spi_panel_suspend(void)
{
	int index;
