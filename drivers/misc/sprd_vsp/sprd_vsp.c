...
static long vsp_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	unsigned char need_rst_axi = 0;
	unsigned int tmp_rst_msk = 0;
	...
	case VSP_RESET:
		...
		if (vsp_hw_dev.version == SHARKL3) {
			need_rst_axi = (readl_relaxed(vsp_glb_reg_base + VSP_AXI_STS_OFF) & 0x7) > 0;
		} else
			need_rst_axi = 0;

		if (need_rst_axi) {
			pr_info("vsp_axi_busy");
			tmp_rst_msk = vsp_reset_mask | BIT(1);
		} else
			tmp_rst_msk = vsp_reset_mask;
