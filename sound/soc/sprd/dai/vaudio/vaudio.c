...
int enable_tuned_clock(void)
{
	if (clk_prepare_enable(clk_tuned_26m)) {
		pr_err("ye.zhang:prepare tuned 26m clock failed!\n");
	}
	pr_err("ye.zhang:enable_tuned_clock successful!\n");
	return 0;
}

int disable_tuned_clock(void)
{
	clk_disable_unprepare(clk_tuned_26m);
	pr_err("ye.zhang:disable_tuned_clock\n");
	return 0;
}
